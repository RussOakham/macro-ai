...
function installCodeQL() {
  if (!isToolInstalled('codeql')) {
    ...
    try {
      ...
      execSync(`curl -L -o codeql.zip "${downloadUrl}"`, { cwd: codeqlDir, stdio: 'inherit' })
      ...
      log('yellow', 'Please install manually: https://codeql.github.com/docs/codeql-cli/getting-started-with-the-codeql-cli/')
      return false
    }
  }
  return true
...
function installOctoscan() {
  if (!isToolInstalled('octoscan')) {
    ...
    try {
      execSync('go install github.com/synacktiv/octoscan@latest', { stdio: 'inherit' })
      log('green', '✅ Octoscan installed')
    } catch (error) {
      log('red', '❌ Failed to install Octoscan. Please install Go and run: go install github.com/synacktiv/octoscan@latest')
      return false
    }
  }
  return true
...
function installSnykScanner() {
  ...
  if (!fs.existsSync(snykDir)) {
    ...
    try {
      ...
      execSync('git clone https://github.com/snyk-labs/github-actions-scanner.git .tools/github-actions-scanner', { 
        cwd: projectRoot, 
        stdio: 'inherit' 
      })
...
function runCodeQLAnalysis() {
  ...
  try {
    ...
    execSync(`${codeqlPath} database create codeql-db --language=actions --source-root=.`, {
      cwd: projectRoot,
      stdio: 'inherit'
    })
    ...
    execSync(`${codeqlPath} database analyze codeql-db --format=sarif-latest --output=codeql-results.sarif`, {
      cwd: projectRoot,
      stdio: 'inherit'
    })
    ...
    if (fs.existsSync(resultsFile)) {
      ...
      if (results.runs && results.runs[0] && results.runs[0].results && results.runs[0].results.length > 0) {
        ...
        results.runs[0].results.forEach(result => {
          ...
          if (result.locations && result.locations[0]) {
            ...
            log('red', `    File: ${location.artifactLocation.uri}:${location.region.startLine}`)
          }
        })
        return false
      } else {
...
function runOctoscanAnalysis() {
  ...
  try {
    ...
    const env = { ...process.env, PATH: `${path.join(projectRoot, '.tools', 'bin')}:${process.env.PATH}` }
...
function runSnykScanner() {
  ...
  try {
    const output = execSync('npm run start -- scan-local --path .github/workflows/', {
      cwd: snykDir,
      encoding: 'utf8',
      stdio: 'pipe'
    })
...
function runActTesting() {
  ...
  if (!isToolInstalled('act')) {
    ...
    try {
      ...
      log('red', '❌ Failed to install act. Please install manually: brew install act')
      return false
    }
  }
...
async function main() {
  ...
  if (!toolsInstalled.every(Boolean)) {
    log('red', '❌ Some required tools could not be installed. Continuing with available tools...')
  }
  ...
  if (allPassed) {
    ...
    log('green', 'Your GitHub Actions workflows are secure and ready to commit.')
    process.exit(0)
  } else {

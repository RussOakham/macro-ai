	...
	for (const tool of tools) {
		if (!isToolInstalled(tool.name)) {
			...
			try {
				...
				log(
					'red',
					`❌ Failed to install ${tool.name}. Please install manually: ${tool.install}`,
				)
				allInstalled = false
			}
		} else {
...
function runActionlint() {
	...
	try {
		const output = execSync(
			'actionlint -color -shellcheck=error -pyflakes=error',
			{
				cwd: projectRoot,
				encoding: 'utf8',
				stdio: 'pipe',
		...
		antiPatterns.forEach((pattern) => {
			...
			if (matches) {
				matches.forEach((match) => {
					...
					log(
						severityColor,
						`❌ ${workflow}:${lineNumber} [${pattern.severity.toUpperCase()}] ${pattern.name}`,
					)
	...
	workflows.forEach((workflow) => {
		...
		if (content.includes('codeql') || content.includes('CodeQL')) {
			...
			if (
				...
				!content.includes('github/codeql-action/init@v4')
			) {
				log(
					'yellow',
					`⚠️ ${workflow}: Consider using latest CodeQL init action`,
				)
				hasIssues = true
			}
			...
			if (
				...
				!content.includes('github/codeql-action/analyze@v4')
			) {
				log(
					'yellow',
					`⚠️ ${workflow}: Consider using latest CodeQL analyze action`,
				)
				hasIssues = true
			}
		} else {
...
function checkDependencyVulnerabilities() {
	...
	try {
		...
		if (!hasLockFile) {
			log(
				'yellow',
				'⚠️ No lock file found. Consider using a lock file for reproducible builds.',
			)
			return true
		}
		...
		if (fs.existsSync(path.join(projectRoot, 'package-lock.json'))) {
			try {
				...
				log(
					'green',
					'✅ npm audit: No moderate or higher vulnerabilities found',
				)
			} catch (error) {

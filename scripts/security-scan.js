...
function installActionlint() {
	if (!isToolInstalled('actionlint')) {
		...
		try {
			...
			log(
				'red',
				'❌ Failed to install actionlint. Please install manually: brew install actionlint',
			)
			return false
		}
	}
...
function installGitleaks() {
	if (!isToolInstalled('gitleaks')) {
		...
		try {
			...
			log(
				'red',
				'❌ Failed to install gitleaks. Please install manually: brew install gitleaks',
			)
			return false
		}
	}
...
function runGitleaks() {
	...
	try {
		const output = execSync(
			'gitleaks detect --source .github/workflows/ --config .gitleaks.toml --verbose',
			{
				cwd: projectRoot,
				encoding: 'utf8',
				stdio: 'pipe',
	...
	workflows.forEach((workflow) => {
		...
		dangerousActions.forEach((action) => {
			if (content.includes(action)) {
				log(
					'red',
					`❌ ${workflow}: Found potentially dangerous action ${action}`,
				)
				hasIssues = true
			}
		})
		...
		lines.forEach((line, index) => {
			secretPatterns.forEach((pattern) => {
				if (
					...
					log(
						'red',
						`❌ ${workflow}:${index + 1} Potential hardcoded secret: ${line.trim()}`,
					)
					hasIssues = true
				}
			})

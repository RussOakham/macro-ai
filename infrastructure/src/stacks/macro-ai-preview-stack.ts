...
export class MacroAiPreviewStack extends cdk.Stack {
	...
	private createPreviewUserData(
		...
		userData.addCommands(
			...
			'',
			'# Copy .env file to application working directory',
			'echo "$(date): Copying .env file to application directory..." >> /var/log/user-data.log',
			'mkdir -p /opt/macro-ai/app',
			'cp /opt/macro-ai/.env /opt/macro-ai/app/.env',
			'chown macroai:macroai /opt/macro-ai/app/.env',
			'chmod 600 /opt/macro-ai/app/.env',
			'echo "$(date): .env file copied to /opt/macro-ai/app/.env" >> /var/log/user-data.log',
			'',

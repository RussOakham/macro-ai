...
const config = {
  ...
  target: 'node20',
  format: 'esm',
  outfile: 'dist/lambda.bundle.mjs',
  external: ['aws-sdk'],
  minify: !isDev,
  sourcemap: true,
  keepNames: true,
  ...
  metafile: true,
  banner: {
    js: '// AWS Lambda ES Module Handler\n'
  },
  footer: {
    js: '\n// Ensure exports are available to Lambda runtime\nexport { handler, initializeExpressApp, initializeServerlessHandler };'
  }
}
...
async function buildLambda() {
  try {
    ...
    console.log(`📊 Bundle size: ${(result.metafile?.outputs?.[config.outfile]?.bytes || 0) / 1024 / 1024} MB`)

...
TEST_AWS_REGION="${AWS_REGION:-us-east-1}"
...
test_backend_api_resolution() {
    ...
    if api_url=$("$resolver_script" --pr-number "$TEST_PR_NUMBER" --region "$TEST_AWS_REGION" --output-format url 2>/dev/null); then
        if [[ -n "$api_url" ]]; then
    ...
    if json_output=$("$resolver_script" --pr-number "$TEST_PR_NUMBER" --region "$TEST_AWS_REGION" --output-format json 2>/dev/null); then
    ...
    if env_output=$("$resolver_script" --pr-number "$TEST_PR_NUMBER" --region "$TEST_AWS_REGION" --output-format env 2>/dev/null); then

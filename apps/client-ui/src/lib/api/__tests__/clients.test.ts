...
describe('API Clients Integration', () => {
	...
	describe('Client Configuration and Setup', () => {
		it('should apply interceptors to unified clients', async () => {
			...
			expect(apiClientWithoutCredentials.instance.interceptors.response).toBeDefined()

			// Verify interceptor handlers are functions
			expect(typeof apiClient.instance.interceptors.response.use).toBe('function')
			expect(typeof apiClientWithoutCredentials.instance.interceptors.response.use).toBe('function')
		})

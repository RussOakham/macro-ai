...
async function generateDomainClients(
	...
	// Domains that use manually maintained schema-based approach
	// These domains have custom Zod schemas in src/schemas/ and should be skipped from auto-generation
	const schemaBasedDomains = ['auth', 'chat', 'user'] as const

	// Domains that should be auto-generated from OpenAPI spec
	// Currently empty as all domains use custom schema-based approach
	const autoGeneratedDomains = domains.filter(
		(domain) => !schemaBasedDomains.includes(domain),
	)

	if (autoGeneratedDomains.length === 0) {
		console.log(
			'All domains use custom schema-based approach - skipping auto-generation',
		)
	}
	...
	for (const domain of domains) {
		...
		if (endpoints.length > 0) {
			...
			// Auto-generate domain client from OpenAPI spec
			console.log(`Auto-generating ${domain} domain client from OpenAPI spec`)
